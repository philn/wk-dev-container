...
RUN git clone -b 1.28 https://gitlab.freedesktop.org/gstreamer/gstreamer && \
    ...
    rm -fr _build gstreamer /usr/lib64/libgst{adaptivedemux,analytics,basecamerabinsrc,cuda,dxva,hip,insertbin,mse,photography,player,rtsp,transcoder,uridownloader,validate,wayland}* && \
